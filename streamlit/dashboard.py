...
selected_indicators = st.sidebar.multiselect(
    ...
    format_func=lambda x: indicators_df[indicators_df['indicator_id']==x]['indicator_name'].iloc[0]
)
...
if len(date_range) == 2:
    data = data[(data['date_key'] >= pd.Timestamp(date_range[0])) & 
...
if len(selected_indicators) > 1:
    ...
    try:
        ...
        deduped_data = data.sort_values('date_key').groupby(['date_key', 'indicator_name'], as_index=False).last()
        
        # Create pivot table
        pivot_data = deduped_data.pivot(index='date_key', columns='indicator_name', values='observation_value')
