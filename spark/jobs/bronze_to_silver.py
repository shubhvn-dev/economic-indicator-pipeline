...
def process_fred_data(spark, bronze_path, silver_path):
    ...
    df_silver = df \
        .withColumn("observation_date", to_date(col("date"))) \
        ...
        .withColumn(
            "is_missing",
            when(col("value") == ".", True).otherwise(False)
        ) \
        .withColumn(
            "is_valid",
            when(col("value_numeric").isNotNull(), True).otherwise(False)
        ) \
        .select(
            col("series_id"),
            col("series_name"),
